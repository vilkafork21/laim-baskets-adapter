...
def test_flat_basket_meets_spec_invariants(tmp_path):
    package = make_package(
        tmp_path, {"Лист1": {"rows": [["q", "a", "m"], ["в1", "о1", 1], ["в2", "о2", 0]]}}
    )
    result = run_package(
        package,
        tmp_path / "out",
        client=FakeClient([layout_answer(), metric_answer()], baseline=baseline_answer()),
    )
...
def test_blob_dialogue_expands_turns(tmp_path):
    ...
    package = make_package(
        tmp_path,
        {"Лист1": {"rows": [["messages", "m"], [blob, 1], ["['КЛИЕНТ: ещё', 'АГЕНТ: снова']", 0]]}},
    )
    result = run_package(
        package,
        tmp_path / "out",
...
def test_classification_accuracy_scores_rows(tmp_path):
    package = make_package(
        ...
        {
            "Лист1": {
                "rows": [["q", "pred", "gt"], ["в1", "кредит", "кредит"], ["в2", "вклад", "кредит"]]
            }
        },
    )
    result = run_package(
...
def test_blank_input_query_rows_are_dropped_not_fatal(tmp_path):
    ...
    package = make_package(
        tmp_path,
        {"Лист1": {"rows": [["q", "a", "m"], ["в1", "о1", 1], [None, "о2", 1], ["в3", "о3", 0]]}},
    )
    result = run_package(
        package,
        tmp_path / "out",
        client=FakeClient([layout_answer()] * 3 + [metric_answer()], baseline=baseline_answer()),
    )
    assert result.status == "computed"
...
def test_metric_failure_degrades_not_dies(tmp_path):
    package = make_package(tmp_path, {"Лист1": {"rows": [["q", "a", "m"], ["в1", "о1", 1]]}})
    bad = metric_answer(
        sources=[
            {
                "column_id": "ZZ",
    ...
    result = run_package(
        package,
        tmp_path / "out",
        client=FakeClient([layout_answer()] + [bad] * 3, baseline=baseline_answer()),
    )
...
def test_ambiguous_baseline_is_not_repaired_or_moved_to_spare_sheet(tmp_path):
    ...
    client = FakeClient(
        [layout_answer(sheet_name="Корзина"), metric_answer()],
        baseline=baseline_answer() + baseline_answer("0.8", "p003"),
    )
...
def test_pinned_sheet_skips_sheet_retry(tmp_path):
    ...
    client = FakeClient([layout_answer()] + [bad] * 3, baseline=baseline_answer())
    result = run_package(package, tmp_path / "out", client=client, sheet_name="Лист1")
    assert result.status == "not_computable"
...
def test_layout_failure_retries_spare_sheet(tmp_path):
    ...
    client = FakeClient(
        [blob, blob, blob, layout_answer(sheet_name="Плоский"), metric_answer()],
        baseline=baseline_answer(),
    )
...
def test_blob_dialogue_role_dicts_expand_turns(tmp_path):
        ...
        tmp_path,
        {
            "Лист1": {
                "rows": [
                    ...
                    [blob, 1],
                    [
                        '[{"role": "user", "content": "ещё"}, {"role": "assistant", "content": "снова"}]',
                        0,
                    ],
                ]
            }
    ...
    assert [(turn[1], turn[2]) for turn in turns] == [
        ("привет", "здравствуйте"),
        ("вопрос", "ответ"),
    ]
...
def test_blob_dialogue_pairs_expand_turns(tmp_path):
    ...
    package = make_package(
        tmp_path, {"Лист1": {"rows": [["dialog", "m"], [blob, 1], ["[('ещё', 'снова')]", 0]]}}
    )
    result = run_package(
        package,
        tmp_path / "out",
        client=FakeClient(
            [_blob_layout("python_list", "Q", "A"), _score_plan()], baseline=baseline_answer()
        ),
    )
    turns = ast.literal_eval(result.umr.frame["dialogue"].iloc[0])
    assert [(turn[1], turn[2]) for turn in turns] == [
...
def test_blob_dialogue_role_dicts_with_russian_roles(tmp_path):
    blob = (
        '[{"speaker": "Клиент", "text": "привет"}, {"speaker": "Оператор", "text": "здравствуйте"}]'
    )
    package = make_package(tmp_path, {"Лист1": {"rows": [["dialog", "m"], [blob, 1], [blob, 0]]}})
    result = run_package(
        package,
        tmp_path / "out",
        client=FakeClient(
...
def test_foreign_validation_report_is_flagged(tmp_path):
    ...
    package = make_package(
        tmp_path,
        {"Лист1": {"rows": [["q", "a", "m"], ["в1", "о1", 1], ["в2", "о2", 0]]}},
    ...
    result = run_package(
        ...
        tmp_path / "out",
        client=FakeClient(
            [layout_answer(), metric_answer()], baseline=baseline_answer(paragraph="p003")
        ),
    )
...
def test_percent_point_scores_publish_ratio_with_warning(tmp_path):
    package = make_package(
        tmp_path,
        {"Лист1": {"rows": [["q", "a", "m"], ["в1", "о1", 70], ["в2", "о2", 100]]}},
    ...
    result = run_package(
        ...
        tmp_path / "out",
        client=FakeClient(
            [layout_answer(), metric_answer(scale="percent")], baseline=baseline_answer("85%")
        ),
    )
...
def test_score_failure_never_tries_spare_sheet_and_keeps_first_basket(tmp_path):
    ...
    client = FakeClient(
        [layout_answer(sheet_name="Корзина")] + [bad] * 3, baseline=baseline_answer()
    )
    result = run_package(package, tmp_path / "out", client=client)
    assert result.status == "not_computable"
    assert result.umr.frame["input_query"].tolist() == ["в1", "в2"]
    assert result.report["decisions"]["sheet"] == "Корзина"
    assert not any(w["code"] == "spec_error" for w in result.report["warnings"])
    assert client.labels.count("layout_turn1") == 1
