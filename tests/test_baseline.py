...
def pick(candidates, paragraphs=('Accuracy 0.93',), **kwargs):
    return select_baseline(candidates, paragraphs, selected_sheet=kwargs.pop('sheet', 'Данные'),
                           metric_name=kwargs.pop('metric_name', 'Accuracy'),
                           scale=kwargs.pop('scale', 'ratio'), **kwargs)
...
@pytest.mark.parametrize('raw,text', [('0.9', 'Accuracy 0.93'), ('9', 'Accuracy 0.9'),
                                      ('0.93', 'Accuracy 10.93'), ('0.9', 'Accuracy 0.9e-3'),
                                      ('0.93', 'Accuracy 0.93%')])
...
def test_key_metric_precedes_hint():
    candidates = [
        dict(mention(), is_key_metric=False), dict(mention('0.85', 'p002'), metric_name='F1')]
...
def test_hint_disambiguates_equal_priority_metrics():
    result = pick([mention(), dict(mention('0.85', 'p002'), metric_name='F1')],
                  ('Accuracy 0.93', 'F1 0.85'))
    assert result.value == Decimal('0.93')


def test_slice_matching_is_independent_of_number_of_workbook_sheets():
    candidates = [dict(mention('0.93'), kind='slice_value', slice_label='  438  '),
...
def test_matching_slice_precedes_general_value():
    result = pick([mention(), dict(mention('0.85', 'p002'), kind='slice_value', slice_label='554')],
                  ('Accuracy 0.93', '554 Accuracy 0.85'), sheet='554')
...
def test_first_number_is_chosen_by_position_not_model_order():
    result = pick([mention('0.85'), mention('0.93'), mention('0.99')],
                  ('Accuracy | 0.93 | 0.85 | 0.99',))
...
def test_rounding_not_a_transitive_cluster():
    result = pick([mention('0.90'), mention('0.9', 'p002'), mention('0.94', 'p003')],
...
def test_threshold_defaults_comparator_and_matches_name():
    result = pick([mention(), dict(mention('0.8', 'p002'), kind='threshold'),
...
def test_threshold_from_other_slice_is_not_used():
    result = pick([mention(), dict(mention('0.8', 'p002'), kind='threshold', slice_label='554')],
