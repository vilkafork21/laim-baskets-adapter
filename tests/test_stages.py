...
def baseline(raw='0.5', paragraph='p002', **fields):
    return [dict(metric_name='Accuracy', raw=raw, paragraph=paragraph,
                 kind='value', slice_label=None, is_key_metric=True, **fields)]
...
@pytest.mark.parametrize('k_response,k_state,k_reason', [
...
def test_failures_never_switch_sheet_and_scores_survive_k_failure(tmp_path, s_ok, k_response, k_state, k_reason):
    root = make_package(tmp_path, {
        'Данные': {'rows': [['q', 'a', 'm'], ['в1', 'о1', 1], ['в2', 'о2', 0]]},
        'Описание': {'rows': [['name', 'info'], ['q', 'вопрос']]},
    }, validation=('Accuracy 0.5', 'Accuracy 0.8'))
    bad = score(sources=[source('ZZ', 'final_score')])
    client = FakeClient([layout_answer(sheet_name='Данные')] +
                        ([score()] if s_ok else [bad] * 3) + [k_response])
...
def test_transport_failure_after_layout_is_not_fatal(tmp_path, fail_label):
    ...
    assert result.km['reason_code'] == ('score_plan_failed' if fail_label == 'metric'
                                        else 'official_baseline_missing')


def test_baseline_prompt_cannot_see_development_or_scores():
    report = {'port': 'validation_report', 'name': 'validation_report.docx',
...
def test_public_descriptor_changes_only_optional_agent_ci_and_sources():
    ...
    clean['script']['runConfiguration']['sourceFiles'].remove('laim_basket/metric/baseline.py')
...
def _formula_package(tmp_path, formula, cached=True):
    ...
    root = make_package(tmp_path, {'Данные': {'rows': [
        ['q', 'a', 'score', 'x', 'y'], ['в1', 'о1', formula, 1, 1],
...
def test_cached_row_formula_is_a_real_score_source(tmp_path):
    client = FakeClient([layout_answer(sheet_name='Данные'), score(), baseline()])
    result = run_package(_formula_package(tmp_path, '=SUM(D2:E2)/2'), tmp_path / 'out', client=client)
    assert result.status == 'computed'
...
def test_nonlocal_or_uncached_formula_does_not_publish_fabricated_score(tmp_path, formula, cached):
    ...
    result = run_package(_formula_package(tmp_path, formula, cached), tmp_path / 'out', client=client)
    assert result.km['reason_code'] == 'score_plan_failed'
    assert result.report['baseline']['state'] == 'declared'
    assert 'main_metric' not in result.umr.frame
...
def test_k_bad_schema_is_repaired_without_rerunning_s(tmp_path):
    client = FakeClient([layout_answer(sheet_name='Данные'), score(),
                         {'state': 'ambiguous'}, baseline()])
    result = run_package(package(tmp_path), tmp_path / 'out', client=client)
    assert result.status == 'computed'
...
def test_k_exhausted_schema_repair_preserves_scores(tmp_path):
    client = FakeClient([layout_answer(sheet_name='Данные'), score()] + [{'state': 'ambiguous'}] * 3)
    result = run_package(package(tmp_path), tmp_path / 'out', client=client)
...
def test_agent_ci_setting_reaches_platform_outputs(tmp_path, monkeypatch):
    ...
    outputs = main.main(root / 'validation_report.docx', root / 'test_set.xlsx',
                        root / 'development_report.docx', root / 'assessor_instruction.txt',
                        agent_ci='CI09774440')
...
def test_k_ambiguity_is_preserved_in_monitoring_metric(tmp_path, monkeypatch):
    import main
    root = make_package(tmp_path, {'Данные': {'rows': [['q', 'a', 'm'], ['в1', 'о1', 1]]}},
                        validation=('Accuracy 0.5', 'Accuracy 0.8'))
    client = FakeClient([layout_answer(sheet_name='Данные'), score(),
                         baseline() + baseline('0.8', 'p003')])
    monkeypatch.setattr(main, 'LlmClient', lambda *args: client)
    outputs = main.main(root / 'validation_report.docx', root / 'test_set.xlsx',
                        root / 'development_report.docx', root / 'assessor_instruction.txt')
