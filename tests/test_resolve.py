...
def test_bounds_stop_at_last_nonempty_input_query(tmp_path):
    ...
    layout = resolve_layout(layout_answer(), _sheets(tmp_path, rows), BASKET, "", frozenset())
...
def test_unknown_sheet_is_layout_error(tmp_path):
    with pytest.raises(LayoutError):
        resolve_layout(
            layout_answer(sheet_name="Нет такого"),
...
def test_non_integer_weight_is_layout_error(tmp_path):
    ...
    with pytest.raises(LayoutError):
        resolve_layout(
            layout_answer(weight_column="C"), _sheets(tmp_path, rows), BASKET, "", frozenset()
        )
