...
def _offset(raw: str, paragraph: str) -> int:
    ...
    for match in re.finditer(re.escape(raw), paragraph):
        if any(
            number.start() < match.start() < number.start() + len(number.group().rstrip())
            or number.start() < match.end() < number.start() + len(number.group().rstrip())
            for number in _NUMBER.finditer(paragraph)
        ):
            continue
...
class BaselineResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "metric_name": self.metric_name or None,
            "scale": self.scale,
            ...
            if self.threshold is None
                ...
                "paragraph": self.threshold_paragraph,
...
def failed_baseline(
    observation: str, *, ambiguous: bool = False, candidates=(), rejected=()
) -> BaselineResult:
    ...
    *,
    ...
    if not values:
        return failed_baseline(
            ...
            + (
                "найдены только значения других срезов"
                if slices
                else "итоговое значение не найдено"
            ),
            ambiguous=bool(slices),
            candidates=verified,
            rejected=rejected,
        )
    ...
    if len({_name(item["metric_name"]) for item in values}) > 1:
        return failed_baseline(
            "имя ключевой метрики не определено однозначно",
            ambiguous=True,
            candidates=verified,
            rejected=rejected,
        )
    # Только в строке таблицы первое значение может отделяться от следующих ДИ.
    # В прозе два разных значения не превращаются в одно из-за общей нумерации.
    first = {}
    for item in sorted(values, key=lambda entry: (int(entry["paragraph"][1:]), entry["position"])):
        paragraph_text = paragraphs[int(item["paragraph"][1:]) - 1]
        key = (item["paragraph"], _name(item["metric_name"]))
    ...
    if any(not _equivalent(left, right) for left, right in combinations(values, 2)):
        return failed_baseline(
            f"{len(values)} разных кандидатов на итоговую КМ",
            ambiguous=True,
            candidates=verified,
            rejected=rejected,
        )
    chosen = min(
        values,
        key=lambda entry: (_comparable(entry)[1], int(entry["paragraph"][1:]), entry["position"]),
    )
    ...
    if thresholds:
        ...
        if same:
            ...
            try:
                threshold = _convert(
                    Decimal(selected["parsed_value"]), selected["scale"], value_scale
                )
        ...
        if not same:
            warnings.append(
                {
                    "code": "threshold_ambiguous",
                    "message": "Этап K. Ожидалось: один порог в согласованной шкале. Получено: "
    ...
    return BaselineResult(
        ...
        threshold_paragraph=threshold_paragraph,
...
def attach_baseline(plan: MeasurementPlan, baseline: BaselineResult) -> MeasurementPlan:
    ...
    threshold = (
        None
        if baseline.threshold is None
        else _convert(baseline.threshold, baseline.scale, plan.scale)
    )
    return replace(
        plan,
        reported_value=value,
        reported_raw=baseline.raw,
