    ...
    for child in node:
        if child.tag == ns + 'p':
            ...
            for row in _children(child, 'tr', ns):
                cells = [' '.join(_blocks(cell, ns, headings=False))
                         for cell in _children(row, 'tc', ns)]
                line = ' | '.join(cells)
                if line.strip(' |'):
                    yield line
        elif child.tag in {ns + 'sdt', ns + 'sdtContent', ns + 'customXml'}:
...
def read_document_paragraphs(path: str | Path, kind: str) -> tuple[str, ...]:
    if kind == 'document_txt':
        ...
        paragraphs = tuple(part.strip().replace('\n', ' ')
        ...
        try:
            ...
            raise ReadError(
                ...
                'Действие: пересохраните документ как корректный DOCX.', path=str(path),
            ) from exc
    else:
