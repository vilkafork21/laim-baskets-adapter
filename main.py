...
from laim_basket.contract import monitoring_metric as _monitoring_metric
...
def _parquet_safe(frame):
    ...
    for column in result.columns:
        ...
        if values.dtype != object:
            continue
        kinds = {type(item).__name__ for item in values if item is not None and item == item}
        if len(kinds) > 1:
            result[column] = [
                None if item is None or item != item else str(item) for item in values
            ]
    return result
...
def main(
    ...
    artifacts = {name: _resolve_artifact(value, name) for name, value in values.items()}
